...
class Test extends ClauseStoragePartitioning with Logging{
  ...
  var hashedges = HashMap[String, EdgeTest]()
  ...
  def edgeoccurence(x: Node, y: Node) = {
    ...
    var edge = new EdgeTest(List[Node](x, y), 1)
   ...
   */
  def findNode(edges: List[EdgeTest], node:Node): Int = {
    var i = 0
    var e = edges
    while(!e.isEmpty){
   ...
   */
  def printGraph(g: List[EdgeTest], file: String) = {
    var i = 0
  ...
  // durch die Methoden edgesTest und edgeoccurence ersetzt
  /*
